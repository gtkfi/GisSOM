...
Python script to split .lrn file to individual data columns, that are stored as numpy binary files.
...
def _read_columns(lrn_header, columns, fmt=''):  #Slightly modified versions of functions by the same names in lrnfile.py
...
def read_csv_columns(lrn_header):
    coord_cols = (0,1)#[i for i, x in enumerate(lrn_header['coltypes']) if x == 0 or x==1 ] 
    fmt = ('%f ' * len(coord_cols)).rstrip()
    return _read_csv_columns(lrn_header, coord_cols, fmt)

if(inputType=="lrn"):                                 
    header=read_lrn_header(inputFile)             #read lrn file
